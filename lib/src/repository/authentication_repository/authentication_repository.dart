...
import 'package:login_flutter_app/src/features/authentication/screens/initial_questions/age_screen.dart';
...
class AuthenticationRepository extends GetxController {
  ...
  void onReady() {
    ...
    decideInitialScreen(_firebaseUser.value);
  ...
  Future<bool> isWeightStored() async {
    ...
    return prefs.getString('weightGoal') != null;
  }

  Future<void> decideInitialScreen(User? user) async {
    final bool hasStoredWeight = await isWeightStored();

    if (!hasStoredWeight) {
      Get.offAll(() =>
          AgeSelectionScreen()); // Navigate to WeightScreen if weight not stored
    } else {
      if (user == null) {
        Get.offAll(() =>
